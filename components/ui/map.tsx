...
type MapProps = {
  ...
  fog?: any; // MapLibre types might be missing FogSpecification explicitly
...
const MapComponent = forwardRef<MapRef, MapProps>(function Map(
  { children, theme: themeProp, styles, projection, fog, ...props },
  ref
) {
  ...
  useEffect(() => {
    ...
    const styleDataHandler = () => {
      ...
      styleTimeoutRef.current = setTimeout(() => {
        ...
        if (fog && typeof (map as any).setFog === 'function') {
            (map as any).setFog(fog);
        } else if (fog) {
            console.warn("map.setFog is not available on this map instance.");
        }
      }, 100);
    };
...
// Memoize the Map component to prevent re-renders when parent state changes
